var siteNameInpt = document.getElementById("siteName"),
  urlInpt = document.getElementById("siteUrl"),
  submit = document.getElementById("submit"),
  tbody = document.querySelector("tbody"),
  tr = document.querySelectorAll("tbody tr"),
  bookMark = [];
function isValidUrl(t) {
  return /(https?:\/\/www\.)?\w+(\.com?)/.test(t);
}
function clear() {
  (urlInpt.value = ""), (siteNameInpt.value = "");
}
function addToArray() {
  var t = { name: siteNameInpt.value, url: urlInpt.value };
  bookMark.push(t), localStorage.setItem("Data", JSON.stringify(bookMark));
}
function displayData(t) {
  tbody.innerHTML = "";
  for (var e = 0; e < t.length; e++)
    tbody.innerHTML += ` <tr id="${e}">
      <td>${e + 1}</td>
      <td>${t[e].name}</td>
      <td>
        <a href="${
          t[e].url.includes("https://www.")
            ? t[e].url
            : "https://www." + t[e].url
...
function sameName(t) {
  for (var e = 0; e < bookMark.length; e++) {
    if (bookMark[e].name === t) return !0;
    return !1;
  }
}
...
localStorage.getItem("Data") &&
  ...
  urlInpt.addEventListener("input", function () {
    var t = isValidUrl(urlInpt.value);
    console.log(t),
      !1 === t
  ...
  document.forms[0].addEventListener("submit", function (t) {
    t.preventDefault();
  }),
  submit.addEventListener("click", function () {
    sameName(siteNameInpt.value),
      "" === urlInpt.value ||
      "" === siteNameInpt.value ||
      !1 === isValidUrl(urlInpt.value) ||
      siteNameInpt.classList.contains("is-invalid")
        ? displayModelError()
        : !0 === sameName(siteNameInpt.value)
        ? modelSameNameErorr()
        : (addToArray(), displayData(bookMark)),
      clear();
  }),
  tbody.addEventListener("click", function (t) {
    t.target.classList.contains("delete") && deleteData(t.target.id);
  });
